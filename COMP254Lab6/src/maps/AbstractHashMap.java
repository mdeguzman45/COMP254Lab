...
public abstract class AbstractHashMap<K,V> extends AbstractMap<K,V> {
  ...
  private double loadFactor;			// Lab 6 Exercise 1
  ...
  /**
   * Lab 6 Exercise 1
   * Creates a hash table with the given capacity and load factor */
  public AbstractHashMap(int cap, double loadFactor) { 
	  // don't create the map if load factor is greater than 1
	  if (loadFactor > 1) {
		  System.out.println("Load factor exceeds maximum allowed: 1");
		  System.exit(0);
	  }
...
//  }
  ...
  /**
   * Lab 6 Exercise 1
   * Creates an updated put method to take load factor into account */
  @Override
  public V put(K key, V value) {
	  V answer = bucketPut(hashValue(key), key, value);
	  if (n > capacity * loadFactor || n > capacity) {            

		  System.out.println();
		  // to see the before and after resize
		  int originalCapacity = capacity;
		  
		  resize(2 * capacity - 1);        // (or find a nearby prime)

		  // print the old and new size
		  System.out.println("Entry #: " + n + "\nAdjusing size from: " + originalCapacity + " to " + capacity);
