...
public class CircularlyLinkedList<E> implements Cloneable {
  ...
  public CircularlyLinkedList<E> clone() throws CloneNotSupportedException {
	  CircularlyLinkedList<E> copyList = new CircularlyLinkedList<E>();
	  
	  if (size > 0 ) {
		  Node<E> originCopy = tail.getNext(); // start from the first node
		  
		  do {
			  copyList.addLast(originCopy.getElement());
			  originCopy = originCopy.getNext();
		  } while(originCopy != tail.getNext()); // stop if we got to the first node again
	  }
...
//main method
  public static void main(String[] args)
  {
	  ...
	  try {
		  CircularlyLinkedList<String> copyCircularList = circularList.clone();
		  System.out.println("Cloned list:");
		  System.out.println(copyCircularList);
	  } catch (CloneNotSupportedException e) {
		  System.out.println("Failed cloning circular linked list");
	  }
