...
public class LinkedBinaryTree<E> extends AbstractBinaryTree<E> {
	 ...
	 * 
	 * The worst case running times for this method is
	 * O(logn) where n is the height of the tree T
	 */
	public Position<E> inorderNext(Position<E> pos) {

		// lets check if the position is internal and have right child
		if (isInternal(pos) && right(pos) != null) {
			pos = right(pos); // this could possibly be the next

			// lets find first occurrence of left leaf
			...
			if (pos == node.left) {
				...
				// loop until the ancestor/parent of the position is a left child of a node - this will be the next
				while (!(pos == node.left)) {
					pos = node;
					node = node.parent;

					// possible that we'll run at the end
					if (node == null) {
						break;
					}
				}
	 ...
	 * postorder traversal of T (or null if p is the last node visited)
	 * 
	 * The worst case running times for this method is
	 * O(logn) where n is the height of the tree T
	 */
	public Position<E> postorderNext(Position<E> pos) {
		...
		// lets check if the position is internal 
		if (isInternal(pos) && parent(pos) != null) {
			// get the parent of position
			node = validate(parent(pos));

			// return the parent node if this is already the right node
			if (pos == node.right) {
				return node;
			} else {
				if (right(pos) != null) {
					// access right sibling then look for the first occurrence of a leaf
					pos = right(node);
					while (!isExternal(pos) && left(pos) != null) {
						pos = left(pos);
					}
				} else {
					// in case right sibling is null
					pos = parent(pos);
				}
	...
	/* 
	 * method that computes and prints, for every position p of a tree T, 
	 * the element of p followed by the height of p’s subtree. 
	 */
	public void printElementAndHeightPosition(Position<E> pos) {
		if (validate(pos) != null) {
			System.out.println("Element: " + pos.getElement() + "[Height = " + height(pos) + "]");
		}

		if (left(pos) != null) {
			printElementAndHeightPosition(validate(left(pos)));
		}

		if (right(pos) != null) {
			printElementAndHeightPosition(validate(right(pos)));
		}
	}
